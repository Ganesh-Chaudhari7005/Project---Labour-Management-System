...
export default function ReportIssues() {
  ...
  const [LabourID] = useState(sessionStorage.getItem("LabourID"));
  const [workingSite, setWorkingSite] = useState({});

  const fetchWorkingSite = async () => {
    try {
      const reqSite = await fetch(
        `${ApiRoute}get-working-site/${LabourID}`
      );
      const res = await reqSite.json();

      if (res.length > 0) {
        setWorkingSite(res[0]);
      }
    } catch (error) {
      console.log(error);
      toast.error("Failed to load working site");
    }
  };
  ...
  const handleIssueSubmit = async (e) => {
    ...
    try {
      const payload = {
        ProjectID: workingSite.ProjectID,
        LabourID,
      ...
      const req = await fetch(`${ApiRoute}insert-Lab-Issue`, {
        method: "POST",
        headers: {
      ...
      >
        ...
        <div className="card p-2">
          <p className="card-text mb-1">
            ...
            {workingSite?.ProjectName || "Loading..."}
          </p>
